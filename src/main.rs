use anyhow::Result;
use commands::{
    handle_get_command, handle_incr_command, handle_psync_command, handle_set_command,
    handle_type_command, handle_wait_command, handle_xadd_command, handle_xrange_command,
...
use std::env;
use std::sync::Arc;
use structs::{ServerInfo, ServerMessageChannels, StoredValue, Value};
...
async fn read_and_handle_single_command_from_local_buffer(
    ...
    *local_buffer = local_buffer[bytes_read..].to_vec();
    ...
    match commands_list[0].as_str() {
        ...
        "incr" => {
            handle_incr_command(stream, storage.clone(), &resp).await?;
        }
        _ => {
